...
public class StaffManager {
	...
	public void insertStaff() {
		...
		int no=dao.selectMaxNo(); // 가장 최근에 등록된 사원
		staffDto.setNo(no+1); // 새로운 사원에게 +1(기존 사원번호에 추가 하려는 작업)
		
		int result=dao.insertStaff(staffDto); // DB 삽입
	...
	public void selectOne() {
		...
		if(staffDto==null) {
			...
			System.out.println(staffDto); // lombok을 했다면 toString()이 이미 애너테이션 되어 있다
		}
	}
